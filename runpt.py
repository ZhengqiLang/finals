...
if __name__ == "__main__":
    ...
    if args.load_ckpt != '':
        ...
        experiment_name = f'date={args.start_datetime}_model={args.model}_p={args.probability_bound}_seed={args.seed}_alg={args.pretrain_method}_distance={args.distance_type}_weights={args.local_weight_type}_sampleNbr={args.local_samples_per_center}'
    ...
    loss_csv_path = output_folder / 'loss_history.csv'
    loss_plot_path = output_folder / 'learning_curve.png'
    radius_plot_path = output_folder / 'radius_curve.png'
    ...
    loss_fieldnames = [
        ...
        'valid_target_frac',
        'valid_decrease_frac',
        'valid_decrease_centers',
    ]

    with loss_csv_path.open('w', newline='', encoding='utf-8') as f:
        writer = csv.DictWriter(f, fieldnames=loss_fieldnames)
        writer.writeheader()

    def _info_scalar(infos_dict, key_name, default=np.nan):
        """Move a scalar JAX value to the host and convert it to float."""
        if key_name not in infos_dict:
            return float(default)
        value = jax.device_get(infos_dict[key_name])
        return float(np.asarray(value))

    def append_loss_row(infos_dict, global_step, cegis_iteration, learner_epoch, batch_idx):
        row = {
            'global_step': int(global_step),
            'cegis_iteration': int(cegis_iteration),
            'learner_epoch': int(learner_epoch),
            'batch': int(batch_idx),
            'mesh_loss': float(args.mesh_loss),
            'total_loss': _info_scalar(infos_dict, '0. total'),
            'init_loss': _info_scalar(infos_dict, '1. init'),
            'unsafe_loss': _info_scalar(infos_dict, '2. unsafe'),
            'decrease_loss': _info_scalar(infos_dict, '3. expDecrease_mean'),
            'n_init_mean': _info_scalar(infos_dict, '9. n_init_mean'),
            'n_unsafe_mean': _info_scalar(infos_dict, '9.1 n_unsafe_mean'),
            'n_target_mean': _info_scalar(infos_dict, '9.2 n_target_mean'),
            'n_decrease_mean': _info_scalar(infos_dict, '9.3 n_decrease_mean'),
            'n_decrease_min': _info_scalar(infos_dict, '9.31 n_decrease_min'),
            'n_decrease_max': _info_scalar(infos_dict, '9.32 n_decrease_max'),
            'valid_init_frac': _info_scalar(infos_dict, '9.4 valid_init_frac'),
            'valid_unsafe_frac': _info_scalar(infos_dict, '9.5 valid_unsafe_frac'),
            'valid_target_frac': _info_scalar(infos_dict, '9.6 valid_target_frac'),
            'valid_decrease_frac': _info_scalar(infos_dict, '9.7 valid_decrease_frac'),
            'valid_decrease_centers': _info_scalar(infos_dict, '9.43 valid_decrease_centers'),
        }
    ...
    def plot_learning_curves():
        """Create loss and RangeNet-radius curves from the current CSV file."""
        try:
            data = np.genfromtxt(loss_csv_path, delimiter=',', names=True, dtype=float)
        ...
        plt.plot(steps, data['total_loss'] + eps, label='Total')
        plt.plot(steps, data['init_loss'] + eps, label='Initial')
        plt.plot(steps, data['unsafe_loss'] + eps, label='Unsafe')
        plt.plot(steps, data['decrease_loss'] + eps, label='Decrease')

        cegis_ids = data['cegis_iteration'].astype(int)
        for cegis_id in np.unique(cegis_ids)[1:]:
            first_idx = np.flatnonzero(cegis_ids == cegis_id)[0]
            plt.axvline(steps[first_idx], linestyle='--', linewidth=0.7, alpha=0.35)
        ...
        plt.figure(figsize=(8, 5))
        plt.plot(steps, data['n_decrease_mean'], label='Mean radius')
        plt.plot(steps, data['n_decrease_min'], label='Minimum radius')
        plt.plot(steps, data['n_decrease_max'], label='Maximum radius')
        plt.xlabel('Learner update step')
        plt.ylabel('Predicted decrease radius')
        plt.legend()
        plt.grid(alpha=0.3)
        plt.tight_layout()
        plt.savefig(radius_plot_path, dpi=300)
        plt.close()
    ...
    for i in range(args.cegis_iterations):
        ...
        for j in tqdm(range(args.epochs), desc=f"Learner epochs (iteration {i})"):
            for k in range(num_batches):
                ...
                # Save one row per learner update. This is outside the jitted
                # train_step, so ordinary file I/O is safe here.
                append_loss_row(
                    infos_dict=infos,
        ...
        # Refresh the learning-curve figures after every CEGIS learner phase.
    ...
    # Ensure that the latest rows are represented in the final plots, even
    # when training terminates because of success or the iteration limit.
    plot_learning_curves()

    if not finished:
        print(f'\n=== Program not terminated in the maximum number of iterations ({args.cegis_iterations}) ===')

    print(f'Learning-curve data saved to: {loss_csv_path}')
    print(f'Loss curve saved to: {loss_plot_path}')
    print(f'Radius curve saved to: {radius_plot_path}')
    print('\n============================================================\n')
