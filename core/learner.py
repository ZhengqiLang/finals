...
'''
Multi-point RangeNet local sampling added to the original learner.

The sampling architecture is kept from the submitted multi-point version:
- one learned radius per centre;
- multiple L1/L2/L-infinity neighbours;
- distance-based weights;
- region masks;
- joint gradients for certificate, policy, and RangeNet.

The loss aggregation is hierarchical:
- first aggregate weighted violations within each centre neighbourhood;
- initial/unsafe then take the maximum across centres;
- decrease takes a mean/RMS across centres, with an optional centre-level maximum;
- counterexamples are merged at the centre level.
'''
import jax
import numpy as np
...
class Learner:
    '''
    Original logRASM learner with RangeNet-based multi-point sampling.

    Sampling:
    - original region allocations determine the total sample budget;
    - the budget is divided into centres and local neighbours;
    - every centre has a learned radius predicted from (x, V(x));
    - local samples use L1, L2, or L-infinity distance;
    - masks retain only samples in the intended region.

    Loss aggregation:
    - weighted mean inside each local neighbourhood;
    - maximum across centres for initial and unsafe conditions;
    - mean/RMS across centres for decrease, with optional centre-level max;
    - the formal verifier is unchanged.
    '''
    ...
    def __init__(self, env, args):
        ...
        if not args.silent:
            ...
            print('- Loss aggregation: hierarchical')
            print('  within neighbourhood = weighted mean')
            print('  across centres: init/unsafe = max; decrease = mean/RMS (+ optional max)\n')
    ...
    def train_step(
        ...
        # SAMPLE-BUDGET MODE
        #
        # Select ONE block by commenting/uncommenting it.
        # The SAME-BUDGET block is active by default.
        # ==============================================================

        # --------------------------------------------------------------
        # INCREASED-BUDGET MODE
        # Original volume-based allocations are used as centre counts.
        # Final samples per region are therefore multiplied by
        # (1 + local_samples_per_center).
        # --------------------------------------------------------------
        ...
        # --------------------------------------------------------------
        # SAME-BUDGET MODE (ACTIVE)
        # Divide each original allocation by the number of samples in a
        # neighbourhood, so centre + neighbours approximately preserve
        # the original total sample budget.
        # --------------------------------------------------------------
        samples_per_neighbourhood = self.local_samples_per_center + 1

        if isinstance(self.num_samples_init, tuple):
            N_init = tuple(
                np.maximum(
                    np.array(self.num_samples_init) // samples_per_neighbourhood,
                    1,
                ).astype(int)
            )
        else:
            N_init = max(
                int(self.num_samples_init // samples_per_neighbourhood),
                1,
            )
        ...
        if isinstance(self.num_samples_unsafe, tuple):
            N_unsafe = tuple(
                np.maximum(
                    np.array(self.num_samples_unsafe) // samples_per_neighbourhood,
                    1,
                ).astype(int)
            )
        else:
            N_unsafe = max(
                int(self.num_samples_unsafe // samples_per_neighbourhood),
                1,
            )
        ...
        if isinstance(self.num_samples_target, tuple):
            N_target = tuple(
                np.maximum(
                    np.array(self.num_samples_target) // samples_per_neighbourhood,
                    1,
                ).astype(int)
            )
        else:
            N_target = max(
                int(self.num_samples_target // samples_per_neighbourhood),
                1,
            )

        N_decrease = max(
            int(self.num_samples_decrease // samples_per_neighbourhood),
            1,
        )
        ...
        def group_by_center(values, num_centers):
            """Reshape flattened neighbourhood samples into (centres, samples_per_centre, ...)."""
            samples_per_center = self.local_samples_per_center + 1
            return values.reshape(
                (num_centers, samples_per_center) + values.shape[1:]
            )

        def hierarchical_weighted_statistics(losses, weights, mask, num_centers):
            """Weighted statistics inside each centre neighbourhood."""
            losses_grouped = group_by_center(jnp.ravel(losses), num_centers)
            weights_grouped = group_by_center(jnp.ravel(weights), num_centers)
            mask_grouped = group_by_center(
                jnp.asarray(mask, dtype=losses_grouped.dtype),
                num_centers,
            )

            valid_weights = weights_grouped * mask_grouped
            weight_sum = jnp.sum(valid_weights, axis=1)
            center_valid = weight_sum > 1e-8
            center_valid_count = jnp.sum(mask_grouped, axis=1)

            center_mean = (
                jnp.sum(valid_weights * losses_grouped, axis=1)
                / (weight_sum + 1e-8)
            )
            center_mean_square = (
                jnp.sum(valid_weights * losses_grouped ** 2, axis=1)
                / (weight_sum + 1e-8)
            )

            center_mean = jnp.where(center_valid, center_mean, 0.0)
            center_mean_square = jnp.where(
                center_valid,
                center_mean_square,
                0.0,
            )
        ...
        def masked_center_max(center_values, center_valid):
            """Maximum over valid centre-level losses."""
            return jnp.max(
                jnp.where(center_valid, center_values, 0.0),
                axis=0,
            )

        def masked_center_mean(center_values, center_valid):
            """Equal-weight mean over valid centres."""
            valid = center_valid.astype(center_values.dtype)
            return (
                jnp.sum(valid * center_values, axis=0)
                / (jnp.sum(valid, axis=0) + 1e-8)
            )

        def masked_center_rms(center_mean_square, center_valid):
            """RMS over valid centre-level weighted mean-square losses."""
            valid = center_valid.astype(center_mean_square.dtype)
            return jnp.sqrt(
                jnp.sum(valid * center_mean_square, axis=0)
                / (jnp.sum(valid, axis=0) + 1e-8)
                + 1e-4
            )
        ...
        def loss_fun(certificate_params, policy_params, range_params):
            ...
            # Hierarchical aggregation: first compress every local
            # neighbourhood into one centre-level statistic.
            (
                init_center_mean,
                init_center_mean_square,
                init_center_valid,
                init_center_valid_count,
            ) = hierarchical_weighted_statistics(
                losses_init,
                weights_init,
                mask_init_valid,
                N_init_centers,
            ...
            (
                unsafe_center_mean,
                unsafe_center_mean_square,
                unsafe_center_valid,
                unsafe_center_valid_count,
            ) = hierarchical_weighted_statistics(
                losses_unsafe,
                weights_unsafe,
                mask_unsafe_valid,
            ...
            (
                decrease_center_mean,
                decrease_center_mean_square,
                decrease_center_valid,
                decrease_center_valid_count,
            ) = hierarchical_weighted_statistics(
                jnp.ravel(Vdiffs),
                weights_decrease,
                valid_nondec_base,
            ...
            if len(counterexamples) > 0:
                ...
                # Weighted mean inside every neighbourhood, then
                # preserve worst-case behaviour across centres.
                base_init_loss = masked_center_max(
                    init_center_mean,
                    init_center_valid,
                )
                base_unsafe_loss = masked_center_max(
                    unsafe_center_mean,
                    unsafe_center_valid,
                )

                # Counterexamples have no local neighbourhood here, so each
                # counterexample acts as a single centre-level observation.
                cx_init_loss = jnp.max(
                    jnp.where(cx_bool_init, losses_init_cx, 0.0),
                    axis=0,
                )
                cx_unsafe_loss = jnp.max(
                    jnp.where(cx_bool_unsafe, losses_unsafe_cx, 0.0),
                    axis=0,
                )

                loss_init = jnp.maximum(base_init_loss, cx_init_loss)
                loss_unsafe = f_unsafe * jnp.maximum(
                    base_unsafe_loss,
                    cx_unsafe_loss,
                )
                ...
                # Hierarchical decrease aggregation. Every valid base
                # neighbourhood contributes one centre-level statistic;
                # every valid counterexample contributes one observation.
                num_valid_centers = jnp.sum(
                    decrease_center_valid.astype(Vdiffs.dtype),
                    axis=0,
                )
                num_valid_cx = jnp.sum(
                    valid_nondec_cx.astype(Vdiffs.dtype),
                    axis=0,
                )
                denom_nondec = num_valid_centers + num_valid_cx + 1e-4

                if self.loss_decr_squared:
                    base_center_sq_sum = jnp.sum(
                        jnp.where(
                            decrease_center_valid,
                            decrease_center_mean_square,
                            0.0,
                    ...
                    cx_sq_sum = jnp.sum(Vdiffs_cx_trim ** 2, axis=0)

                    loss_exp_decrease_mean = expDecr_multiplier * (
                        jnp.sqrt(
                            (base_center_sq_sum + cx_sq_sum)
                            / denom_nondec
                            + 1e-4
                        )
                        - 1e-2
                    ...
                    cx_sum = jnp.sum(Vdiffs_cx_trim, axis=0)

                    loss_exp_decrease_mean = expDecr_multiplier * (
                        (base_center_sum + cx_sum) / denom_nondec
                    )

                if self.loss_decr_max:
                    base_center_max = masked_center_max(
                        decrease_center_mean,
                        decrease_center_valid,
                    )
                    cx_max = jnp.max(
                        jnp.where(valid_nondec_cx, Vdiffs_cx, 0.0),
                        axis=0,
                    )
                    loss_exp_decrease_max = jnp.maximum(
                        base_center_max,
                        cx_max,
                    )
                else:
                ...
                # Weighted mean inside each neighbourhood, then maximum
                # across centres for initial and unsafe conditions.
                loss_init = masked_center_max(
                    init_center_mean,
                    init_center_valid,
                )
                loss_unsafe = f_unsafe * masked_center_max(
                    unsafe_center_mean,
                    unsafe_center_valid,
                )

                # Equal-weight mean/RMS across valid centres for decrease.
                if self.loss_decr_squared:
                    loss_exp_decrease_mean = expDecr_multiplier * (
                        masked_center_rms(
                            decrease_center_mean_square,
                ...
                if self.loss_decr_max:
                    loss_exp_decrease_max = masked_center_max(
                        decrease_center_mean,
                        decrease_center_valid,
                    )
                else:
            ...
            infos = {
                ...
                '9.81 valid_decrease_weight_sum': jnp.sum(weights_valid_decrease),
                ...
                '9.41 valid_init_centers': jnp.sum(init_center_valid),
                '9.42 valid_unsafe_centers': jnp.sum(unsafe_center_valid),
                '9.43 valid_decrease_centers': jnp.sum(decrease_center_valid),
                '9.44 max_init_center_loss': masked_center_max(
                    init_center_mean,
                    init_center_valid,
                ),
                '9.45 max_unsafe_center_loss': masked_center_max(
                    unsafe_center_mean,
                    unsafe_center_valid,
                ),
                '9.46 mean_decrease_center_loss': masked_center_mean(
                    decrease_center_mean,
                    decrease_center_valid,
                ),
                '9.47 max_decrease_center_loss': masked_center_max(
                    decrease_center_mean,
                    decrease_center_valid,
                ),
                '9.48 mean_valid_local_decrease': jnp.mean(
                    decrease_center_valid_count
                ),
