...
class Learner:
    ...
    def train_step(self,
        ...
        def sample_neighbors(self,center,key,n,local_n):
            ...
            if self.local_distance_type == "inf":
            ...
            if self.local_distance_type == "l1":
...
# ------------------------------------------------------------
            ...
            samples_decrease_bool_not_Unsafe = self.env.unsafe_space.jax_not_contains(samples_decrease)
            samples_decrese_bool_not_init = self.env.init_space.jax_not_contains(samples_decrease)

            samples_init_bool_not_target = self.env.init_space.jax_not_contains(samples_init)
            samples_init_bool_not_Unsafe = self.env.init_space.jax_not_contains(samples_init)
            samples_init_bool_not_decrease = self.env.init_space.jax_not_contains(samples_init)

            samples_unsafe_bool_not_target = self.env.unsafe_space.jax_not_contains(samples_unsafe)
            samples_unsafe_bool_not_init = self.env.unsafe_space.jax_not_contains(samples_unsafe)
            sample_unsafe_bool_not_decrease = self.env.unsafe_space.jax_not_contains(samples_unsafe)
            ...
            if len(counterexamples) > 0:
                ...
                # mean init / unsafe ！！！！
                base_init_mean = jnp.sum(weights_init * losses_init, axis=0)
                base_unsafe_mean = jnp.sum(weights_unsafe * losses_unsafe, axis=0)
...
# ------------------------------------------------------------
        ...
        samples_decrease_bool_not_targetUnsafe_log = self.env.target_space.jax_not_contains(
            samples_decrease_log
        )
