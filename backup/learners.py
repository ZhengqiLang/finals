...
class Learner:
    ...
    def train_step(self,
        ...
        base_sample_decrease = self.env.state_space.sample(rng=decrease_key,N=1)
        ...
        def sample_neighbors(self,center,key,n):
            print("用了center",self.local_n)
            D = self.env.state_dim

            if self.local_distance_type == "l2":
                key_dir, key_r = jax.random.split(key, 2)
                dirs = jax.random.normal(key_dir, shape=(self.local_n, D))
                dirs = dirs / (jnp.linalg.norm(dirs, axis=1, keepdims=True) + 1e-8) #get direction vertor in 1
                u = jax.random.uniform(key_r, shape=(self.local_n, 1), minval=0.0, maxval=1.0) #probability u=pi*r^2/pi*n^2
                r = n * (u ** (1.0 / D))  # r
            ...
            if self.local_distance_type == "l1":
                ...
                idx = jnp.where(mask, size=self.local_n, fill_value=0)[0]
                offsets = cand[idx]
        ...
        def loss_fun(certificate_params, policy_params,range_params):
            ...
            samples_decrease, dists = sample_neighbors(
                self, center=base_sample_decrease, key=decrease_key, n=n
            )
            ...
            if self.local_weight_type == "inverse":
                ...
                # 高斯核权重
                sigma = 0.3 * (n) + 1e-6
                weights = jnp.exp(-0.5 * (dists / sigma) ** 2)

            elif self.local_weight_type == "inverse_gaussian":
                # 反比 + 高斯，两个都要
                sigma = self.local_gaussian_sigma_scale * n + 1e-6
                ...
                weights_gauss = jnp.exp(-0.5 * (dists / sigma) ** 2)

                weights = weights_inv * weights_gauss
            ...
            if len(counterexamples) > 0:
                ...
                weights_valid = weights*valid_nondec
            ...
            alpha_n = 1e-3
            beta_n = 1e-3
            loss_n_reg = alpha_n / (n + 1e-6) + beta_n * (n ** 2)

            # 2) weights 熵正则：防止权重过度集中到少数点
            #    H(w) = - sum_i w_i log w_i
            #    在 loss 里加入 negative entropy => sum_i w_i log w_i
            lambda_entropy = 1e-4
            loss_entropy = lambda_entropy * jnp.sum(weights * jnp.log(weights + 1e-8))
