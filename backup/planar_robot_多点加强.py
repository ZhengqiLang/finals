...
class PlanarRobot(BaseEnvironment, gym.Env):
    ...
    def sample_noise(self, key, size=None):
        return jnp.zeros((self.noise_dim,))
    ...
    def sample_noise_numpy(self, size=None):
        return np.zeros((self.noise_dim,))
    ...
    #         return np.zeros((self.noise_dim,), dtype=np.float32)
    ...
    #     w = jnp.zeros((self.noise_dim,), dtype=state.dtype)

    #     state_mean = self.step_base(state, u, w)

    #     epsilon = jnp.zeros_like(state_mean)
    ...
    #     prob = np.ones((n,), dtype=np.float32) / n
