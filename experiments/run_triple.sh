...
--expDecr_multiplier 50 \
--logger_prefix Triplestoincrease=10 \
--mesh_verify_grid_init 0.04 \
--local_samples_per_center 10 \
--noise_partition_cells 6"
...
do
  ...
  do
    ...
    do
      ...
      do
        # if [ "${distance_type}" = "l2" ] && [ "${local_weight_type}" = "inverse" ] && [ "${seed}" = "3" ]; then

        #   echo "Skip already finished setting: distance_type=${distance_type}, local_weight_type=${local_weight_type}"
