...
def ffilter(p):
    ...
    while len(p.ast_ns) > 0:
        ...
        b.append(-p.tev[1][i])
        sol = solvers.lp(matrix([0.0] * len(X)), matrix(np.array(A)), matrix(b), solver='glpk',
...
def branch(p, sgn):
    ...
    res = solvers.lp(matrix([0.0] * len(X)), matrix(np.array(A)), matrix(b), solver='glpk',
                     options={'glpk': {'msg_lev': 'GLP_MSG_OFF'}})
...
def LipBaB(wts, bs, bnds):
    ...
    bnds = [[float(x) for x in l] for l in bnds]
    print(bnds)
    ...
    pnorm = 1  # norm of choice (1,2,inf)
    ...
    while (True):
        ...
        print(tp.Lub, glb, time_now)
        if time_now > 600: break
        if tp.Lub <= af * glb:
            break
        branch(tp, -1)
        branch(tp, 1)
